...
const CATEGORY_COLORS = {
  foundations: "bg-amber-50 text-amber-600",
  skeleton: "bg-cyan-500/15 text-blue-600",
  walls: "bg-emerald-500/15 text-emerald-600",
  ...
  return (
    <div className={`eng-card overflow-hidden border-r-4 ${statusBorder}`}>
      <button
        ...
        className="w-full flex items-center gap-3 p-3.5 hover:bg-black/[0.02] transition-colors text-right"
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded border border-slate-200 text-slate-400 bg-white">{element.type}</span>
            {element.count > 1 && <span className="inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded border border-blue-200 text-blue-600 bg-blue-50">×{element.count}</span>}
            {element.is_typical && <span className="inline-flex items-center text-[10px] font-medium px-1.5 py-0.5 rounded border border-emerald-500/30 text-emerald-600 bg-emerald-50">טיפוסי</span>}
          </div>
      ...
      {expanded && (
        <div className="border-t border-slate-200 p-4 space-y-3 bg-white/50">
          {/* Missing data alert */}
          {elIssues.length > 0 && (
            <div className={`p-3 rounded border ${elStatus === "missing" ? "bg-rose-50 border-rose-500/20" : "bg-amber-50 border-amber-200"}`}>
              <p className={`text-xs font-semibold mb-1.5 ${elStatus === "missing" ? "text-rose-600" : "text-amber-600"}`}>
              ...
              <ul className="space-y-1">
                {elIssues.map((issue, i) => (
                  <li key={i} className="flex items-start gap-2 text-xs">
                    ...
                    <div className={elStatus === "missing" ? "text-rose-300/80" : "text-amber-600/80"}>
                      <strong>{issue}</strong>
              ...
              <Button
                ...
                className={`h-7 text-[11px] gap-1 mt-2 rounded ${elStatus === "missing" ? "border-rose-500/30 text-rose-600 hover:bg-rose-50" : "border-amber-200 text-amber-600 hover:bg-amber-50"}`}
              >
          ...
          {dimEntries.length > 0 && (
            <div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {dimEntries.map(([k, v]) => (
                  <div key={k} className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    ...
                    {editing ? (
                      <Input
                        ...
                        className="h-6 text-xs mt-0.5 p-1 bg-slate-50 border-slate-200 text-slate-800"
                      />
                    ) : (
                      <span className="text-xs font-semibold text-slate-800 eng-number">{formatDim(v, dims.unit || 'ס"מ')}</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          {isValidValue(reinf.raw_text) && (
            <div>
              ...
              <p className="text-xs text-slate-400 bg-white p-2.5 rounded border border-slate-200 font-mono leading-relaxed">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {isValidValue(reinf.main_bars?.top) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    <span className="text-[10px] text-slate-500">עליון</span>
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.main_bars.top)}</span>
                  </div>
                )}
                {isValidValue(reinf.main_bars?.bottom) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    <span className="text-[10px] text-slate-500">תחתון</span>
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.main_bars.bottom)}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {isValidValue(reinf.stirrups?.diameter) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    ...
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.stirrups.diameter)}</span>
                  </div>
                )}
                {isValidValue(reinf.stirrups?.spacing) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    <span className="text-[10px] text-slate-500">ריווח</span>
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.stirrups.spacing)} ס"מ</span>
                  </div>
                )}
                {isValidValue(reinf.stirrups?.spacing_zones) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200 col-span-3">
                    <span className="text-[10px] text-slate-500">אזורים</span>
                    <span className="text-xs font-semibold text-slate-800 block">{cleanValue(reinf.stirrups.spacing_zones)}</span>
                  </div>
                )}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {isValidValue(reinf.lap_splice?.length) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    ...
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.lap_splice.length)} ס"מ{isValidValue(reinf.lap_splice.location) ? ` (${cleanValue(reinf.lap_splice.location)})` : ""}</span>
                  </div>
                )}
                {isValidValue(reinf.bends?.angle) && (
                  <div className="bg-white px-2.5 py-2 rounded border border-slate-200">
                    <span className="text-[10px] text-slate-500">כיפוף</span>
                    <span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.bends.angle)}°{isValidValue(reinf.bends.leg_length) ? ` רגל: ${cleanValue(reinf.bends.leg_length)}` : ""}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {reinf.mesh && (isValidValue(reinf.mesh.direction_x) || isValidValue(reinf.mesh.direction_y)) && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {isValidValue(reinf.mesh.direction_x) && <div className="bg-white px-2.5 py-2 rounded border border-slate-200"><span className="text-[10px] text-slate-500">כיוון X</span><span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.mesh.direction_x)}</span></div>}
                {isValidValue(reinf.mesh.direction_y) && <div className="bg-white px-2.5 py-2 rounded border border-slate-200"><span className="text-[10px] text-slate-500">כיוון Y</span><span className="text-xs font-semibold text-slate-800 block eng-mono">{cleanValue(reinf.mesh.direction_y)}</span></div>}
              </div>
            </div>
          )}

          {isValidValue(element.grid_location) && (
            <div className="text-xs text-slate-500">מיקום על צירים: <span className="font-semibold text-slate-700 eng-mono">{cleanValue(element.grid_location)}</span></div>
          )}

          {isValidValue(element.notes) && (
            <p className="text-xs text-amber-600/80 bg-amber-50 p-2.5 rounded border border-amber-200">{cleanValue(element.notes)}</p>
          )}
          ...
          <div className="flex gap-2 pt-1">
            {editing ? (
              <>
                ...
                <Button size="sm" variant="outline" onClick={() => { setEditing(false); setCorrections({}); }} className="h-8 text-xs gap-1 rounded border-slate-200 text-slate-400 hover:bg-white/5"><X className="w-3 h-3" />ביטול</Button>
              </>
            ) : (
              <Button size="sm" variant="outline" onClick={() => setEditing(true)} className="h-8 text-xs gap-1 rounded border-slate-200 text-slate-400 hover:text-amber-600 hover:border-amber-200 hover:bg-amber-50"><Pencil className="w-3 h-3" />תקן מידות</Button>
            )}
          </div>
        </div>
      )}
