...
const CATEGORY_COLORS = {
  ...
  concrete: "bg-slate-500/15 text-slate-700 border-slate-500/30",
  steel: "bg-cyan-500/15 text-blue-600 border-blue-200",
  formwork: "bg-orange-500/15 text-orange-400 border-orange-500/30",
  masonry: "bg-stone-500/15 text-stone-400 border-stone-500/30",
  plaster: "bg-emerald-500/15 text-emerald-600 border-emerald-500/30",
...
export default function PriceCard({ item, onUpdate, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-2 sm:gap-3 bg-white rounded border border-slate-200 px-3 py-2.5 sm:px-4 sm:py-3">
        ...
        {editing ? (
          <>
            <Input
              ...
              className="w-20 h-7 sm:h-8 text-xs sm:text-sm text-left bg-slate-50 border-slate-200 text-slate-800"
              dir="ltr"
            />
            <Button size="icon" variant="ghost" className="h-6 w-6 text-emerald-600 hover:bg-emerald-50" onClick={handleSave}>
          ...
          <>
            <span className="text-sm sm:text-base font-bold text-amber-600 min-w-[60px] text-left eng-number" dir="ltr">
              ₪{item.price.toLocaleString()}
            </span>
            <Button size="icon" variant="ghost" className="h-7 w-7 text-slate-500 hover:text-amber-600 hover:bg-amber-50" onClick={() => setEditing(true)}>
              <Pencil className="w-3 h-3" />
            </Button>
            <Button size="icon" variant="ghost" className="h-7 w-7 text-slate-500 hover:text-rose-600 hover:bg-rose-50" onClick={() => onDelete(item.id)}>
